...
import com.speedoring.modal.vendor.vendor_product_list.VendorProductListing;
import com.speedoring.ui.user.activity.UserProductDetailActivity;

import java.util.ArrayList;
import java.util.List;

public class VendorListingPaginationAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private List<VendorProductListing> productListings;
    ...
    public VendorListingPaginationAdapter(Context context, View.OnClickListener onClickListener) {
        productListings = new ArrayList<>();
        this.context = context;
        this.onClickListener = onClickListener;
    }

    public List<VendorProductListing> getProductList() {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        switch (getItemViewType(position)) {
            case ITEM:
                ...
                if (productListings.get(position).getProductImages().size() > 0) {
                    Glide.with(context)
                            .load(productListings.get(position).getProductImages().get(0).getProdcutImage())
    ...
    /*Helper pagination*/
    public void add(VendorProductListing r) {
        productListings.add(r);
        notifyItemInserted(productListings.size() - 1);
    }

    public void addAll(List<VendorProductListing> moveResults) {
        for (VendorProductListing result : moveResults) {
            add(result);
        }
    }
    ...
    public void removeLoadingFooter() {
        ...
        VendorProductListing result = getItem(position);
    ...
    public VendorProductListing getItem(int position) {
