package com.speedoring.utils.wave_view;
...
public class WaveView extends View {
    ...
    public static class WaveData {
        ...
        public WaveData(float waveLength, float waveHeight, float fixedHeight, float offset,
                        int startColor, int endColor, float alpha, long duration, boolean right) {
...
//                            Log.d(TAG, "onAnimationUpdate");
    ...
    private void resetPath(WaveData waveData, int w, int h) {
        if (waveData.gradientOrientation == WaveData.GRADIENT_ORIENTATION_HORIZONTAL) {
            waveData.shader = new LinearGradient(0, 0, w, 0, waveData.startColor,
                    waveData.endColor, Shader.TileMode.REPEAT);
        } else {
            waveData.shader = new LinearGradient(0, h - waveData.getFixedHeight() - waveData.getWaveHeight(),
                    0, h - waveData.fixedHeight, waveData.startColor, waveData.endColor, Shader.TileMode.REPEAT);
        }
...
//                Log.d(TAG, "cubic to " + currentPointX + ", " + currentPointY);
    ...
    private void drawWave(Canvas canvas, WaveData waveData) {
        float translation = USE_ANIMATION && waveData.valueAnimator != null ?
                waveData.valueAnimator.getAnimatedFraction() * waveData.getWaveLength() * (waveData.right ? 1 : -1)
                        + waveData.offset : waveData.offset;
    ...
    public void addDefaultWaves(int right, int left, Context mContext) {
        for (int i = 0; i < right; i++) {
            addWaveData(new WaveView.WaveData((float) (800 + Math.random() * 100),
                    (float) (100 + Math.random() * 20), (float) (200 + Math.random() * 20),
                    (float) (Math.random() * 50), mContext.getResources().getColor(R.color.gray_h),
                    mContext.getResources().getColor(R.color.gray_i), 0.3f,
                    (long) (2000 + Math.random() * 1000), true));
        }
        ...
        for (int i = 0; i < left; i++) {
            addWaveData(new WaveView.WaveData((float) (800 + Math.random() * 100),
                    ...
                    (float) (Math.random() * 50), mContext.getResources().getColor(R.color.gray_h),
                    mContext.getResources().getColor(R.color.gray_i), 0.3f,
                    (long) (2000 + Math.random() * 1000), false));
        }
    }
